...
if __name__ == "__main__":
    ...
    # print(df_avg_city_plot)

    # you decide the sample number here
    if p.sample_number == 0:
        sample_number = len(temp_df_names[temp_df_names['label'] == 'male'].example_label.unique())
    ...
    for c_idx, c in enumerate(df_avg_city_plot.city.unique()):
        g = sns.barplot(x="price_lvl", y="ratio", hue='label',
                        data=df_avg_city_plot[(df_avg_city_plot['bias'] == 'gender') & (df_avg_city_plot['city'] == c)], ax=axes[c_idx])
        # Neutralization line
    ...
    fig.text(0.08, 0.5, 'to gender', rotation='vertical', fontsize=16)
    fig.text(0.46, 0.07, 'price level ($)', fontsize=16)

    if p.save_figure:
        temp_file_path = p.saveFigure_dir + 'price_level_ratio_All_Cities2.pdf'
